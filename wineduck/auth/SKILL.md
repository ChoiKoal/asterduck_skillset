# WineDuck Auth — 사용자 인증 스킬

WineDuck 서비스 사용자 인증을 처리하는 스킬.
로그인, 회원가입, 토큰 관리를 담당하며, 다른 WineDuck 스킬들의 인증 기반이 됨.

## Trigger

다음 키워드에 반응: "로그인", "회원가입", "인증", "계정", "비밀번호 변경", "wineduck auth", "wineduck login"
...
로그인 후 받은 토큰은 다른 WineDuck API 호출 시 헤더에 포함:
```
...
1. 사용자가 인증이 필요한 기능 요청 시:
   - "WineDuck 계정이 있어? 로그인할까, 아니면 새로 가입할까?"
2. 로그인 시:
...
| role | 설명 | 권한 |
|------|------|------|
| user | 일반 사용자 | 본인 테이스팅 노트 CRUD, 와인 등록 |
